...
public class Instituition {
	...
	public void instituitionIdnull() {
		...
		LoginRequest logininstituitionIdnull = getLoginAPIReq_instituitionIdNull();

		System.out.println(logininstituitionIdnull);

		request.body(logininstituitionIdnull);
		...
		Assert.assertEquals(loginResponse.getResponseData().getRequestReferenceNo(),
				logininstituitionIdnull.getRequestData().getRequestReferenceNo());
		Assert.assertTrue(responseReferenceNoValidation);
	}

	@Test
	public void instituitionIdlength() {
		...
		LoginRequest logininstituitionIdlength = getLoginAPIReq_instituitionIdLength();

		System.out.println(logininstituitionIdlength);

		request.body(logininstituitionIdlength);
		Response response = request.post("https://products.fss.co.in/CMSAPI/login");
		System.out.println(response.asString());
		int code = response.getStatusCode();
		Assert.assertEquals(code, 200);
		...
		Assert.assertEquals("", loginResponse.getResponseData().getAuthValue());
		Assert.assertEquals(loginResponse.getResponseData().getRequestReferenceNo(),
				logininstituitionIdlength.getRequestData().getRequestReferenceNo());
		Assert.assertTrue(responseReferenceNoValidation);

	}

	@Test
	public void instituitionIddatabase() {
		...
		LoginRequest logininstituitionIddatabase = getLoginAPIReq_instituitionIddatabase();

		System.out.println(logininstituitionIddatabase);

		request.body(logininstituitionIddatabase);
		Response response = request.post("https://products.fss.co.in/CMSAPI/login");
		System.out.println(response.asString());
		int code = response.getStatusCode();
		Assert.assertEquals(code, 200);
		...
		Assert.assertEquals("", loginResponse.getResponseData().getAuthValue());
		Assert.assertEquals(loginResponse.getResponseData().getRequestReferenceNo(),
				logininstituitionIddatabase.getRequestData().getRequestReferenceNo());
		Assert.assertTrue(responseReferenceNoValidation);
	...
	static RequestDataForm getRequestData_institutionIdNull(String reqType) {
	...
	static LoginRequest getLoginAPIReq_instituitionIdNull() {
		LoginRequest loginReq = new LoginRequest();

		loginReq.setRequestData(getRequestData_institutionIdNull("LA"));
	...
	static RequestDataForm getRequestData_institutionIdLength(String reqType) {
		RequestDataForm reqData = new RequestDataForm();
		reqData.setBranchCode("20002");
		reqData.setRequestType(reqType);
		reqData.setRequestDate(getDate());
		reqData.setRequestTime(getTime());
		reqData.setRequestReferenceNo(getRefNo(reqType, reqData.getRequestDate(), reqData.getRequestTime()));//
		reqData.setInstitutionId(10011);
		reqData.setChannelIdentifier("IB");
		reqData.setUserCategory(1);
		reqData.setUserId("41000182");
		reqData.setServiceKey("NA");
	...
	static LoginRequest getLoginAPIReq_instituitionIdLength() {
		LoginRequest loginReq = new LoginRequest();

		loginReq.setRequestData(getRequestData_institutionIdLength("LA"));
	...
	static RequestDataForm getRequestData_institutionIddatabase(String reqType) {
		RequestDataForm reqData = new RequestDataForm();
		reqData.setBranchCode("20002");
		reqData.setRequestType(reqType);
		reqData.setRequestDate(getDate());
	...
	static LoginRequest getLoginAPIReq_instituitionIddatabase() {
		LoginRequest loginReq = new LoginRequest();

		loginReq.setRequestData(getRequestData_institutionIddatabase("LA"));
