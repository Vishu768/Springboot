...
public class UserId {
	...
	public void userIdnull() {
		...
		LoginRequest loginuserIdnull = getLoginAPIReq_userIdNull();

		System.out.println(loginuserIdnull);

		request.body(loginuserIdnull);
		...
		Assert.assertEquals(loginResponse.getResponseData().getRequestReferenceNo(),
				loginuserIdnull.getRequestData().getRequestReferenceNo());
	...
	public void userIdlength() {
		...
		LoginRequest loginuserIdlength = getLoginAPIReq_userIdLength();

		System.out.println(loginuserIdlength);

		request.body(loginuserIdlength);
		Response response = request.post("https://products.fss.co.in/CMSAPI/login");
		System.out.println(response.asString());
		int code = response.getStatusCode();
		Assert.assertEquals(code, 200);
		...
		Assert.assertEquals("", loginResponse.getResponseData().getAuthValue());
		Assert.assertEquals(loginResponse.getResponseData().getRequestReferenceNo(),
				loginuserIdlength.getRequestData().getRequestReferenceNo());
		Assert.assertTrue(responseReferenceNoValidation);
	}

	@Test
	public void userIddatabase() {
		...
		LoginRequest loginuserIddatabase = getLoginAPIReq_userIddatabase();

		System.out.println(loginuserIddatabase);

		request.body(loginuserIddatabase);
		Response response = request.post("https://products.fss.co.in/CMSAPI/login");
		System.out.println(response.asString());
		int code = response.getStatusCode();
		Assert.assertEquals(code, 200);
		...
		Assert.assertEquals("", loginResponse.getResponseData().getAuthValue());
		Assert.assertEquals(loginResponse.getResponseData().getRequestReferenceNo(),
				loginuserIddatabase.getRequestData().getRequestReferenceNo());
		Assert.assertTrue(responseReferenceNoValidation);
	}
	...
	static RequestDataForm getRequestData_userIdNull(String reqType) {
	...
	static LoginRequest getLoginAPIReq_userIdNull() {
		LoginRequest loginReq = new LoginRequest();

		loginReq.setRequestData(getRequestData_userIdNull("LA"));
	...
	static RequestDataForm getRequestData_userIdlength(String reqType) {
		RequestDataForm reqData = new RequestDataForm();
		reqData.setBranchCode("20002");
		reqData.setRequestType(reqType);
		reqData.setRequestDate(getDate());
		...
		reqData.setChannelIdentifier("IB");
		reqData.setUserCategory(1);
		reqData.setUserId("410014125412589746310");
		reqData.setServiceKey("NA");
		return reqData;
	}

	static LoginRequest getLoginAPIReq_userIdLength() {
		LoginRequest loginReq = new LoginRequest();

		loginReq.setRequestData(getRequestData_userIdlength("LA"));
	...
	static RequestDataForm getRequestData_userIddatabase(String reqType) {
		RequestDataForm reqData = new RequestDataForm();
		reqData.setBranchCode("20002");
		reqData.setRequestType(reqType);
		reqData.setRequestDate(getDate());
		...
		reqData.setChannelIdentifier("IB");
		reqData.setUserCategory(1);
		reqData.setUserId("0000000");
		reqData.setServiceKey("NA");
		return reqData;
	}

	static LoginRequest getLoginAPIReq_userIddatabase() {
		LoginRequest loginReq = new LoginRequest();

		loginReq.setRequestData(getRequestData_userIddatabase("LA"));
